...
// Timeout in milliseconds for detecting a system freeze
const uint32_t systemTimeout = 5000; // Adjust based on system needs
...
// Array to track the last valid time for each touch pin
unsigned long lastValidTime[8] = {0};

// Tracks the last overall valid system time
...
void setup() {
  ...
  // Initialize last system valid time
...
void loop() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    // Update the last valid time if the value is within normal range
    if (touchValue < 900000) {  // Ensure the value is not stuck or frozen
      lastValidTime[i] = millis();
      systemFrozen = false; // System is working properly
    }
    ...
    if (touchValue > thresholds[i]) {
      ...
      // Read the analog sensor value and map it
  ...
  // Check if the system is frozen based on timeout
  if (systemFrozen) {
    Serial.println("System frozen! Restarting...");
    delay(100); // Give time to flush serial output
    ESP.restart(); // Force microcontroller restart
  }
